#include "PHG4PSDHcalDetector.h"
#include "PHG4PSDHcalDisplayAction.h"
...
PHG4PSDHcalDetector::PHG4PSDHcalDetector(PHG4Subsystem* subsys, PHCompositeNode* Node, PHParameters* parameters, const std::string& dnam)
  : PHG4Detector(subsys, Node, dnam)
  , m_DisplayAction(dynamic_cast<PHG4PSDHcalDisplayAction*>(subsys->GetDisplayAction()))
  , m_Params(parameters)
...
//_______________________________________________________________________
int PHG4PSDHcalDetector::IsInPSDHcal(G4VPhysicalVolume* volume) const
{
...
//_______________________________________________________________________
void PHG4PSDHcalDetector::ConstructMe(G4LogicalVolume* logicWorld)
{
  if (Verbosity() > 0)
  {
    std::cout << "PHG4PSDHcalDetector: Begin Construction" << std::endl;
  }

  if (m_Params->get_string_param("mapping_file").empty())
  {
    cout << "ERROR in PHG4PSDHcalDetector: No mapping file specified. Abort detector construction." << endl;
  ...
  m_DisplayAction->AddVolume(hcal_envelope_log, "PSDHcalEnvelope");
...
//_______________________________________________________________________
G4LogicalVolume*
PHG4PSDHcalDetector::ConstructTower()
{
  if (Verbosity() > 0)
  {
    std::cout << "PHG4PSDHcalDetector: Build logical volume for single tower..." << std::endl;
  }
...
//                                              TowerDy / 2.0 /*pDy2*/, WlsDw/2.0 /*pDx3*/, WlsDw/2.0 /*pDx4*/, 0.*degree  /*pAlp2*/);
  ...
  if (Verbosity() > 0)
  {
    std::cout << "PHG4PSDHcalDetector: Building logical volume for single tower done." << std::endl;
  }

  return single_tower_logic;
}

int PHG4PSDHcalDetector::PlaceTower(G4LogicalVolume* hcalenvelope, G4LogicalVolume* singletower)
{
  ...
  for (std::map<std::string, towerposition>::iterator iterator = m_TowerPostionMap.begin(); iterator != m_TowerPostionMap.end(); ++iterator)
  {
    if (Verbosity() > 0)
    {
      std::cout << "PHG4PSDHcalDetector: Place tower " << iterator->first
...
int PHG4PSDHcalDetector::ParseParametersFromTable()
{
  ...
  {
    std::cout << "ERROR in PHG4PSDHcalDetector: Failed to open mapping file " << m_Params->get_string_param("mapping_file") << std::endl;
    gSystem->Exit(1);
  }
  ...
  while (getline(istream_mapping, line_mapping))
  {
    ...
    if (line_mapping.find("#") != string::npos)
    {
      if (Verbosity() > 0)
      {
        std::cout << "PHG4PSDHcalDetector: SKIPPING line in mapping file: " << line_mapping << std::endl;
      }
      continue;
    }
    ...
    if (line_mapping.find("Tower ") != string::npos)
    {
      ...
      {
        cout << "ERROR in PHG4PSDHcalDetector: Failed to read line in mapping file " << m_Params->get_string_param("mapping_file") << std::endl;
        gSystem->Exit(1);
      }

...
PHG4PSDHcalSteppingAction::PHG4PSDHcalSteppingAction(PHG4PSDHcalDetector* detector, const PHParameters* parameters)
...
PHG4PSDHcalSteppingAction::~PHG4PSDHcalSteppingAction()
{
...
//____________________________________________________________________________..
bool PHG4PSDHcalSteppingAction::UserSteppingAction(const G4Step* aStep, bool)
{
  G4TouchableHandle touch = aStep->GetPreStepPoint()->GetTouchableHandle();
  G4VPhysicalVolume* volume = touch->GetVolume();
  // m_Detector->IsInPSDHcal(volume)
  // returns
  //  0 is outside of Forward HCAL
  //  1 is inside scintillator
  // -1 is inside absorber (dead material)

  int whichactive = m_Detector->IsInPSDHcal(volume);
...
//     std::cout << touch->GetVolume(i)->GetCopyNo() << "\t";
//   }
  std::cout << std::endl;
  int idx_j = icopy >> 16;
  int idx_k = icopy & 0xFFFF;
  int idx_l = (int)(layer / 10);
  ...
  if (m_ActiveFlag)
  {
    ...
    if (whichactive > 0)
    {
      ...
      if (once && edep > 0)
      {
        ...
        if (Verbosity() > 0)
        {
          std::cout << "PHG4PSDHcalSteppingAction::UserSteppingAction::"
                    //
                    << m_Detector->GetName() << " - "
...
//____________________________________________________________________________..
void PHG4PSDHcalSteppingAction::SetInterfacePointers(PHCompositeNode* topNode)
{
  ...
  if (!m_HitContainer)
  {
    std::cout << "PHG4PSDHcalSteppingAction::SetTopNode - unable to find " << hitnodename << std::endl;
  }
  if (!m_AbsorberHitContainer)
  {
    if (Verbosity() > 0)
    {
      std::cout << "PHG4PSDHcalSteppingAction::SetTopNode - unable to find " << absorbernodename << std::endl;
    }
  }
}
